...
1. **Create PowerShell Script on DC-00**:
   - Save as `C:\Scripts\Monitor-Backup.ps1`:
     ...
     ```powershell
     # Load Windows Server Backup module
     ...
     try {
         ...
         if ($backupStatus) {
             foreach ($job in $backupStatus) {
                 Write-Log "Backup Job: $($job.JobState), Time: $($job.StartTime)"
                 Write-Host "Backup Job: $($job.JobState), Time: $($job.StartTime)"
                 if ($job.JobState -ne "Completed") {
                     Write-Log "Warning: Backup job not completed! State: $($job.JobState)"
                     Write-Host "Warning: Backup job not completed! State: $($job.JobState)"
                 }
             }
         } else {
         ...
         # Check last backup
         $lastBackup = Get-WBBackupStatus
         Write-Log "Last Backup: $($lastBackup.LastBackupTime)"
         Write-Host "Last Backup: $($lastBackup.LastBackupTime)"
     }
     catch {
     ...
     Write-Host "Backup monitoring completed! Check $logPath for details."
     ```

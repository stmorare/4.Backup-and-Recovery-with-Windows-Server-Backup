...
## Overview
This project implements a backup and recovery solution using Windows Server Backup on DC-00 to protect the `SalesFolder` on FS-01 within the `mydomain.local` domain. 
The process includes configuring a backup job for a replicated `SalesFolder` on DC-00, 
simulating data loss by renaming the original `SalesFolder` to `SalesFolder_Old` on FS-01, restoring data to an alternate location on DC-00 due to a 
greyed-out "Original location" option, and manually overwriting `SalesFolder_Old` on FS-01. It also automates backup monitoring with PowerShell and 
verifies the restore on the Windows 11 client, with documentation uploaded to GitHub.
